...
import { createTodoType, deleteTodoType, updateTodoType } from "./todo.schema";
...
export async function getTodosHandler(_: Request, res: Response) {
  try {
...
export async function createTodoHandler(
  ...
  try {
    ...
    res.send(omit(todo.toObject(), ["createdAt", "__v"]));
  } catch (e) {
    res.status(400).send(e);
  }
}
